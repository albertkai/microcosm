...
if (Meteor.isClient) {
  this.bigFive = {
    calc: function() {
      var answer, answers, prop, res, _primary, _primaryPercentage, _secondary, _secondaryPercentage;
      log('calc');
      res = {};
      ...
      for (answer in answers) {
        if (answers[answer] > 80) {
          res[answer] = 5;
        } else if (answers[answer] <= 80 && answers[answer] > 60) {
          res[answer] = 4;
        } else if (answers[answer] <= 60 && answers[answer] > 40) {
          res[answer] = 3;
        } else if (answers[answer] <= 40 && answers[answer] > 20) {
          res[answer] = 2;
      ...
      for (prop in _primaryPercentage) {
        _primaryPercentage[prop] = _primaryPercentage[prop] / 75;
      }
      for (prop in _secondaryPercentage) {
      ...
      return Meteor.call('saveBigFive', Meteor.user()._id, _primaryPercentage, _secondaryPercentage, function(err, res) {
        if (err) {
          log('results are not saved:(');
          return log(err);
        } else {
          return log('BigFive results saved!');
        }
      });
    }
  };
...
if (Meteor.isServer) {
  Meteor.methods({
    'saveBigFive': function(id, primary, secondary) {
      Meteor.users.update(id, {
        $set: {
          'profile.tests.bigFive.results.primary': primary
        }
      });
      return Meteor.users.update(id, {
        $set: {
          'profile.tests.bigFive.results.secondary': secondary
        }
      });
    }
  });
...
//# sourceMappingURL=e33f295ef678b6dbc77e446100f8b00465f8eb37.map
